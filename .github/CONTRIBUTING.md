...
**Issue・ブランチ・コミット・Pull Request の運用ルール**をまとめたものです。
...
- 1 Issue に複数ラベルを付けてよい
- ラベルは「役割」＋「作業カテゴリ」の2軸で考える
- 迷い始めたらラベルを増やさない
...
| ラベル                    | 内容                                            |
| ------------------------- | ----------------------------------------------- |
| `category: frontend 🎨`  | フロントエンド・UI/UX・画面実装の変更           |
| `category: backend 🧠`   | API・ビジネスロジック・アプリケーション層の変更 |
| `category: infra ☁️`    | インフラ・CI/CD・デプロイ・運用基盤の変更       |
| `category: docs 📚`      | README・設計資料・運用ドキュメントの変更        |
| `category: dev-env 🧰`   | 開発環境・VSCode拡張・lint・formatterの変更     |
| `category: refactor ♻️` | リファクタ・構造改善・技術的負債の解消          |
...
- `<description>`：対応概要を Kebab Case で設定

> 例：
> - feature/#123-add-pokedex-filter
> - fix/#45-fix-flag-not-updafte
